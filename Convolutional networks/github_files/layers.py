...
class FC(Layer):
    ...
    def backward_params(self, grads):
        ...
        """
        grad_weights = np.dot(np.transpose(grads), self.prev_inputs)
...
class SoftmaxCrossEntropyWithLogits():
    ...
    def forward(self, x, y) -> float:
        ...
        probs: ndarray = scipy.special.softmax(x, axis=1)
        indices = np.argwhere(y == 1)[:, 1]
        correct_probs: ndarray = probs[np.arange(len(x)), indices]
...
class L2Regularizer():
    ...
    def forward(self):
        ...
        """
        return (self.weight_decay / 2) * (self.weights*self.weights).sum()
