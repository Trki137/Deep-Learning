...
def draw_conv_filters(epoch, step, layer, save_dir, weight_decay, layer_i):
    C = layer.C
    ...
    filename = '%s_epoch_%02d_step_%06d_input_%03d_conv_%d' % (layer.name, epoch, step, i, layer_i)
    if weight_decay is not None:
        log = '../zadatak2/logs/mnist/filters'
        filename = '%s_reg_%07f.png' % (filename, weight_decay)
    else:
        log = '../zadatak1/logs/mnist/filters'
        filename = filename + ".png"

    ski.io.imsave(os.path.join(save_dir, filename), img_as_uint(img))
    writer = SummaryWriter(log)
    writer.add_images(filename, img, dataformats='HW')
    writer.close()



def train(train_x, train_y, valid_x, valid_y, net, loss, config, weight_decay=None):
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for i in range(num_batches):
            ...
            if i % 100 == 0:
                draw_conv_filters(epoch, i * batch_size, net[0], save_dir, weight_decay, 1)
                # draw_conv_filters(epoch, i*batch_size, net[3], save_dir,2)
            if i > 0 and i % 50 == 0:
